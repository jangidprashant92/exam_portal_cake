...
class AclPermissionsComponent extends Component
{
	...
	{			
		$this->_controller->Auth->authenticate = array(
		 ...
		 $this->_controller->Auth->authError = __d('admin', '');
		...
		{						
		    ...
		    if ( $this->_controller->Auth->user() )
		    {
		    	...
		    	$id = ClassRegistry::init('Group')->field('id', array('alias' => 'public'));
		    	ClassRegistry::init('Group')->id = $id;
		    	$groupID =  ClassRegistry::init('Group')->exists() ? $id : 3;
		    }
